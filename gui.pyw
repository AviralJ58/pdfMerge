...
def display_files(location):
    ...
    listNodes = Listbox(frm, width=50, yscrollcommand=scrollbar.set)
    ...
    for filename in os.listdir("."):
        ...
        if (filename.endswith('.pdf') or filename.endswith('.ppt') or filename.endswith('.pptx') or filename.endswith('.doc') or filename.endswith('.docx')):
            ...
            if (filename.endswith('.ppt') or filename.endswith('.pptx')):
                ...
                # l=Label(root,bg='#282C34',fg='white',text=f'{count}. {temp}')
...
lab=Label(root,bg='#282C34',fg='white',text="File Directory:  "+location)
lab.grid(row=0,column=0)
display_files(location)

lab=Label(root,bg='#282C34',fg='white',text='FILES IN ORDER').grid(column=0,row=5,columnspan=3,sticky=W,padx=10)

r = StringVar()
Label(root,bg='#282C34',fg='white',text="Want to merge all files?").grid(column=0,row=1,sticky=E+W+N)
Radiobutton(root,text="Yes",variable=r,value="Y",bg='#282C34',fg='green').grid(row=1,column=1, sticky=E+W+N)
Radiobutton(root,text="No",variable=r,value="N",bg='#282C34',fg='red').grid(row=1,column=2, sticky=E+W+N)

Label(root,bg='#282C34',fg='white',text="Output Filename:").grid(column=0,row=2)
e=Entry(root,text="filename",width=40,borderwidth=5)
e.grid(row=2,column=1,columnspan=3, sticky=E+W+N)

Label(root,bg='#282C34',fg='white',text="If all files are not to be merged, enter file no. in order separated by ',' :").grid(column=0,row=3, sticky=E+W+N)
k=Entry(root,text="fileno",width=40,borderwidth=5)
k.grid(row=3,column=1,columnspan=3,sticky=E+W+N)

Button(text="Merge",bg="grey",fg="white",command=lambda:select_files(r.get())).grid(column=1,row=4, sticky=E+W+N)

Button(text="Compress",bg="grey",fg="white",command=lambda:compressConf()).grid(column=2,row=4, sticky=E+W+N)

root.mainloop()


#aviral isko try karna change karne jo icon aata hai na task bar me use kaise change kare hume nahi aa raha and i dontknow how to fix it